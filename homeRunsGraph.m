...
%%%%%Our reference "Let It Eat" is a well known baseball slang term that can mean anything from OK to lets do this/give everything youve got?start it off etc. This played in well as the animate button because it starts the sliderbar.
end
...
function [] = plotSelectedYear(type) %whichever button was selected is the plot that shows up on the gui graph
global gui; %access gui
...
%%%%use an if statement to show the proper values depending on what button
%%%%gets pushed. 
    ...
    %%%%call the gui plot and plot the current year vs yearly homeruns as
    %%%%red x's
    ...
    gui.p = plot(currentYear:currentYear + 9,yearlyHomeRuns,'rx'); %plots the current year to the current year plus 9 years ex. 2000:2009
     ...
     %%%%add labels to the graph so user understands what they are looking
     %%%%at
